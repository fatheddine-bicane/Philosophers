...
/*   Updated: 2025/06/27 18:25:22 by fbicane          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_status(t_philo_stat stat, t_philosopher *philo)
{
	...
	if (TAKEN_A_FORK == stat && false == end_dinner(philo->table))
		printf(YELLOW"%-6ld"BLUE" %d"RESET " has taken a fork\n", curent_time, philo->id);
	else if (EATING == stat && false == end_dinner(philo->table))
		printf(YELLOW"%-6ld"BLUE" %d"RESET " is eating\n", curent_time, philo->id);
	else if (SLEEPING == stat && false == end_dinner(philo->table))
		printf(YELLOW"%-6ld"BLUE" %d"RESET " is sleeping\n", curent_time, philo->id);
	else if (THINKING == stat && false == end_dinner(philo->table))
		printf(YELLOW"%-6ld"BLUE" %d"RESET " is thinking\n", curent_time, philo->id);
	else if (DIED == stat)
		printf(YELLOW"%-6ld"BLUE" %d" RED" died\n"RESET, curent_time, philo->id);
